...
def calculate_radiation_pressure(
    luminosity: float, distance: float, reflectivity: float
) -> float:
    return (1+reflectivity)*luminosity/4/pi/c * (cos(AVERAGE_RADIATION_ANGLE)*AU/distance)**2


def calculate_total_gravitational_acceleration(
    spaceship: Any, bodies: dict[str, Any]
...
def calculate_gravitational_acceleration_from_body(
    spaceship: Any, body: Any
) -> tuple[float, float, float]:
...
def escape_velocity(body_mass: float, distance: float) -> float:
    return sqrt(2 * G * body_mass / distance)


def delta_v_to_establish_orbit(
    spaceship_pos: tuple[float, float, float],
    spaceship_vel: tuple[float, float, float],
    body: Any,
    eccentricity: float,
    direction: str = DEFAULT_ORBIT_DIRECTION,
) -> tuple[float, float, float]:
    r = (spaceship_pos[0] - body.x, spaceship_pos[1] - body.y, spaceship_pos[2] - body.z)
    tangential_vector = return_tangential_vector(vector=r, direction=direction, normalized=True)
    distance = return_vector_module(
        spaceship_pos[0] - body.x,
        spaceship_pos[1] - body.y,
        spaceship_pos[2] - body.z,
    )
    orbital_velocity = orbital_velocity_module(body.mass, distance, eccentricity)
    orbital_velocity_vector = (
        tangential_vector[0] * orbital_velocity,
...
def orbital_velocity_module(
    planet_mass: float, distance: float, eccentricity: float = 0
) -> float:
    semi_major_axis = distance / sqrt(1 - eccentricity**2)
    orbital_velocity = sqrt(G * planet_mass * ((2 / distance/1000) - (1 / semi_major_axis/1000))) / 1000    # In km/s
    return orbital_velocity


def orbital_period(planet_mass: float, radius_or_semi_major_axis: float) -> float:
    return 2 * pi * sqrt(radius_or_semi_major_axis**3 / G / planet_mass)


def simulate_spaceship_trajectory(
    self: Any,
    start_time: datetime.datetime,
    input_vector: list[tuple[float, float, float, float, float]],
) -> None:
...
def hohmann_transfer(
    body: Any, spaceship: Any, r2: float
) -> tuple[float, float, float]:
    body_position = body.return_position_vector()
    spaceship_position = spaceship.return_position_vector()
    initial_distance_vector = spaceship_position - body_position
    r1 = return_vector_module(
        initial_distance_vector.x,
...
def vector_from_to(
    base_vector: tuple[float, float, float],
...
def return_tangential_vector(
    ...
    tangential_vector = rotate_vector(vector=vector, rotate_z=rotation, normalized=normalized)
    return tangential_vector


def return_vector_module(x: float, y: float, z: float) -> float:
    return sqrt(x**2 + y**2 + z**2)


def return_normalized_vector(x: float, y: float, z: float) -> tuple[float, float, float]:
