...
def hex_to_color(hex_color: str) -> Color:
    ...
    return ursina_color.rgba(r, g, b, a=1.0)
...
def vec3_to_scene(v: Vec3) -> UrsinaVec3:
    return ecliptic_to_scene(x_km=v.x,
                             y_km=v.y,
                             z_km=v.z)
...
def make_ring_mesh(inner: float,
                   outer: float,
                   segments: int = RING_SEGMENTS) -> Mesh:
    """
