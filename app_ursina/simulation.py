...
class SolaraApp(MissionMixin, HudMixin, MenuMixin, Entity):
    ...
    def __init__(self, ephemeris: JplEphemeris, bodies: dict, epoch: datetime) -> None:
        ...
        self.plan_text = Text(text="", position=(-0.85, 0.22), scale=0.7,
                              origin=(-0.5, 0.5), color=ursina_color.white,
                              enabled=False)
        self.help_text = Text(
    ...
    def advance(self, dt_s: float) -> None:
        ...
        if self.moon_trip:
            if self._moon_returning and self.sim_time_s >= self._moon_return_at:
                ...
                relative_position = self.sim_ship.position - self.bodies["Earth"].position
                relative_velocity = self.sim_ship.velocity - self.bodies["Earth"].velocity
    ...
    def _follow_entity(self):
        ...
        name = self.follow_names[self.follow_index]
        if name == SHIP_NAME:
